...
class Parser:
    ...
    def sem_err(Self,msg):
        if self.err_dist >= Parser.min_err_dist:
            errors.sem_err(t.line, t.col, msg)
            self.err_dist = 0
    ...
    def game_state_expr(self):
        ...
        pm_str = self.ident()
        self.expect(5)
        if not playmode.PlayModeUtil.try_parse(pm_str):
            self.sem_err('Unable to parse play mode ' + pm_str + ' .')
        else:
            play_mode = playmode.PlayModeUtil.try_parse(pm_str)
        self.expect(5)

        return time, play_mode, player_id, team_side
    ...
    def univseral_joint_expr(self):
        ...
        self.expect(5)
        self.expect(5)
        return UniversalJointState(label, angle1, angle2)
    ...
    def touch_state_expr(self):
        ...
        self.expect(5)
        return TouchState(label, is_touching)
    ...
    def force_state_expr(self):
        ...
        self.expect(5)
        self.expect(5)
        return ForceState(label,point, force)
    ...
    def visible_item_expr(self):
        ...
        if label == 'F1L':
            self.see_landmark(pos, Landmark.FlagLeftTop)
        elif self.la.kind == 'F2L':
            self.see_landmark(pos, Landmark.FlagLeftBottom)
        elif self.la.kind == 'F1R':
            self.see_landmark(pos, Landmark.FlagRightTop)
        elif self.la.kind == 'F2R':
            self.see_landmark(pos, Landmark.FlagRightBottom)
        elif self.la.kind == 'G1L':
            self.see_landmark(pos, Landmark.GoalLeftTop)
        elif self.la.kind == 'G2L':
            self.see_landmark(pos, Landmark.GoalLeftBottom)
        elif self.la.kind == 'G1R':
            self.see_landmark(pos, Landmark.GoalRightTop)
        elif self.la.kind == 'G2R':
            self.see_landmark(pos, Landmark.GoalRightBottom)
        elif self.la.kind == 'B':
    ...
    def player_expr(self):
        ...
        while self.la.kind == 7:
            ...
            pos = self.polar_pos_expr()
            self.expect(5)
            parts.append(BodyPartPosition(part_label, pos))

        is_team_mate = team_name == self.team_name
        player = PlayerPosition(is_team_mate, int(player_id),parts)
    ...
    def line_expr(self):
        ...
        self.visible_lines.append(VisibleLine(end1,end2))
    ...
    def hear_expr(self):
        ...
        self.expect(5)
        return HeardMessage(time,direction,Message(message_text))
